...
object Param {
  implicit def apply(v: Short): ShortParam = new ShortParam(v)
  implicit def apply(v: Int): IntParam = new IntParam(v)
  implicit def apply(v: Long): LongParam = new LongParam(v)
  implicit def apply(v: Double): DoubleParam = new DoubleParam(v)
  implicit def apply(v: String): StringParam = new StringParam(v)
  implicit def apply(v: java.util.Date): TimestampParam = new TimestampParam(v)
  implicit def apply(v: java.sql.Date): DateParam = new DateParam(v)
  implicit def apply(v: BigDecimal): BigDecimalParam = new BigDecimalParam(v)
  implicit def apply(v: Boolean): BooleanParam = new BooleanParam(v)
}
