...
case class StringParam(v: String) extends ValueParam[String] {
  def set(ps: PreparedStatement, index: Int) { ps setString (index, v) }
}

case class ShortParam(v: Short) extends ValueParam[Short] {
  def set(ps: PreparedStatement, index: Int) { ps setShort (index, v) }
}

case class IntParam(v: Int) extends ValueParam[Int] {
  def set(ps: PreparedStatement, index: Int) { ps setInt (index, v) }
}

case class LongParam(v: Long) extends ValueParam[Long] {
  def set(ps: PreparedStatement, index: Int) { ps setLong (index, v) }
}

case class DoubleParam(v: Double) extends ValueParam[Double] {
  def set(ps: PreparedStatement, index: Int) { ps setDouble (index, v) }
}

case class DateParam(v: java.util.Date) extends ValueParam[java.util.Date] {
  def set(ps: PreparedStatement, index: Int) { ps setTimestamp (index, new Timestamp(v.getTime)) }
}
...
case class BigDecimalParam(v: BigDecimal) extends ValueParam[BigDecimal] {
  ...
  def set(ps: PreparedStatement, index: Int) { ps setBigDecimal (index, new java.math.BigDecimal(v.toString)) }
}
...
object Param {
  ...
  implicit def apply(v: java.util.Date) = new DateParam(v)
